    ...
    for i,feat_x in enumerate(self.embedding_features):
        for j in np.arange(i+1,len(self.embedding_features)):
            ...
            for name_trace in self.trace_name:
                ...
                while (len(remaining_indices)>0):
                    ...
                    if (bool_cost) :
                        if (new_cost>1 and last_cost<1):
                            fractions_thres_cl.append(len(selected_indices)/n_values)
                ...
                fractions_dict[(feat_x,feat_y)][name_trace] = self.df_trace[name_trace].index.to_numpy()[selected_indices]
                fractions_dict[(feat_y,feat_x)][name_trace] = self.df_trace[name_trace].index.to_numpy()[selected_indices]
