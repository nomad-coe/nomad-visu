    ...
    def __init__( self, Figure, visualizerUtilsWidgets, visualizerViewersWidgets ):
        ...
        def utils_button_clicked( button):
            ...
            if Figure.path_to_structures:
                if visualizerUtilsWidgets.widg_box_utils.layout.visibility == "visible":
                    visualizerUtilsWidgets.widg_box_utils.layout.visibility = "hidden"
                    for i in range(340, -1, -1):
                        visualizerViewersWidgets.widg_box_viewers.layout.top = str(i) + "px"
                    visualizerUtilsWidgets.widg_box_utils.layout.bottom = "0px"
                else:
                    for i in range(341):
                        visualizerViewersWidgets.widg_box_viewers.layout.top = str(i) + "px"
                    visualizerUtilsWidgets.widg_box_utils.layout.bottom = "400px"
                    visualizerUtilsWidgets.widg_box_utils.layout.visibility = "visible"
            else:
                if visualizerUtilsWidgets.widg_box_utils.layout.visibility == "visible":
                    visualizerUtilsWidgets.widg_box_utils.layout.visibility = "hidden"
                else:
